...
- =flags= or modifiers
  ...
  - =/m= multiline
    ...
    - [[file:../attachments/Pasted-image-20240318225325.png]]
  - =/i= case-Insensitive
- =capture groups=
  - [[https://youtu.be/c9HbsUSWilw?si=nx8eHI1J1ukAvF6v][ref]]
...
- =Non-capturing Grouping= {to disable referencing this group} using =?:= #still/regex
  - [[https://www.youtube.com/watch?v=eEGUUVvrgdo][23. Understand Captured and Non Captured Groups in Regular Expression - RegEx]]
...
#+begin_quote
=globbing= = translating the wild card chars like {=* ? . ^ [some chars here]=} - remember that =* or ?= .. does not match with the (leading dot) - بمعنى لو جربت تعمل مثلا =ls *something= - never shows something like =.vimrc= .. it has a leading dot
#+end_quote
