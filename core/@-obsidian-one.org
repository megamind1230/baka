...
- a good book to read about note taking & making [[file:how-to-take-smart-notes-by-sonke-ahrens.org][how-to-take-smart-notes-by-sonke-ahrens]]
  ​# simple MarkDown.. h1
...
/italic =_=/
/same as =*text*=/
*/=___=italic bold again lol/*

==highlight== =ctrl j= you still can change this color from a [[file:obsidian-css.org][simple obsidian css file]]
...
- A trick in the unorder list: to ignore the =-= and display it as it is - just use =\= to escape the =-=
...
- =alt up, down= > move line
  - =alt shift up , down= > [[file:obsidian-plugin-{outliner}.org][outliner]]: move list with its children up, down
- =note composer=
...
* images
=![[Pasted image 20240318225325.png]]= - [[file:../attachments/Pasted-image-20240318225325.png]] - image should be local - {which means:inside your obsidian vault} - better specify a folder for then - eg: {image_attachments} - to resize the image just use =|= then the dimensions like this - =![[Pasted image 20240318225325.png|300]]= width is 300px - =![[Pasted image 20240318225325.png|300x50]]= now height is 50
## how to resize all images in a file to a specific size - =%s/png/png|350/g= using vim replace/substitute
# tags - no whitespaces in between words - can be nested with =/= ..to create child tags
eg:
...
3. embedded link with either =!= or =html ifram tag=
   ...
   [[file:@-obsidian-plugins-MOC.org][just a {backlink} to some local note]]
...
* external links
- =ctrl k= > make an external link quickly [[https://youtube.com/shorts/bmrnb3Cj90I?feature=share][text]]
  - =[]()=
    ​# embed links
...
* [[file:0-best-fonts-to-distinguish-between-characters.org][0-best-fonts-to-distinguish-between-characters]]
