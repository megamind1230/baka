...
- [x] bash script to alias as comp to compile current cpp file into file a binary
	...
	- [x] simple bash line to compile a single cpp file
		- `alias com='g++ -std=c++14 main.cpp -o a && ./a'` now you can from vscode do like `ctrl shift c comp` to open terminal in current file path and voila
- [x] use system clipboard .. works by default
	- [x] yank whole file with `:%y`
# settings.json
```json
{
