...
- cpp new approach of array
  =c     #include <array>     ...     array<int, 5> n{1,2,3,6,7};      // same as: int n[5] = {1,2,3,6,7}     //array<int, 5> n{}; all 0's=
...
- a
  =c     //loop through rows of grades array     //grades = the whole array, student = a single row     for (auto const& student : grades) {          // loop through columns of current row         for (auto const& grade : student) {//grade = a single exam test grade=
